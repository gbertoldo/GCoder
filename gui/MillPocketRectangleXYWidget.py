...
import subroutines.MillPocketRectangleXY 

class MillPocketRectangleXYWidget(Frame):
    def __init__(self, master):
        ...
        self.fields.append(  FloatEntry(name="LX",                                    label="Largura do retangulo em X (mm ou in)", value=50.0, lower=FLOATLOWERBOUND, upper=FLOATUPPERBOUND))
        self.fields.append(  FloatEntry(name="LY",                                    label="Largura do retangulo em Y (mm ou in)", value=30.0, lower=FLOATLOWERBOUND, upper=FLOATUPPERBOUND))
        ...
        self.fields.append(  FloatEntry(name="Passo maximo em XY",                    label="Passo maximo nas direcoes X e Y (mm ou in)", value=1.0, lower=0.0, upper=FLOATUPPERBOUND))
        ...
        self.fields.append(  FloatEntry(name="Passo de acabamento em XY",             label="Passo de acabamento nas direcoes X e Y (mm ou in)", value=1.0, lower=0.0, upper=FLOATUPPERBOUND))
    ...
    def getGCode(self):
        ...
        LX   = extractValue('LX',fields)
        LY   = extractValue('LY',fields)
        Zsafe = extractValue('Z seguro',fields)
        Zi = extractValue('Z inicial',fields)
        DZ = extractValue('Profundidade do corte',fields)
        Zf = Zi-DZ
        stepXY = extractValue('Passo maximo em XY',fields)
        stepZ = extractValue('Passo em Z',fields)
        dXYfinishing = extractValue('Passo de acabamento em XY',fields)
        dZfinishing = extractValue('Passo de acabamento em Z',fields)
        feedrateZ = extractValue('Taxa de corte em Z',fields)
        feedrateXY = extractValue('Taxa de corte em X e Y',fields)
        toolDiameter = extractValue('Diametro da fresa',fields)
        contourDir = extractValue('Sentido do corte',fields)
    
        gcode = subroutines.MillPocketRectangleXY.millPocketRectangleXY(Xref, Yref, LX, LY, Zi, Zf, Zsafe, stepXY, stepZ, dXYfinishing, dZfinishing, feedrateXY, feedrateZ, toolDiameter, contourDir)

        return gcode

if __name__ == "__main__":
    app = MillPocketRectangleXYWidget(None)
    app.grid()
