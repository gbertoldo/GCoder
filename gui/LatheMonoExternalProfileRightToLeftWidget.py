...
class LatheMonoExternalProfileRightToLeftWidget(Frame):
    def __init__(self, master):
        ...
        # Loading the image
        self.img = PIL.Image.open(os.path.dirname(os.path.abspath(__file__))+"/fig/TurningExternalRightToLeft.png") 
        ratio = 1.0
        maxSize = 300.0
    ...
    def getGCode(self):
        ...
        gcode = LatheMonotonicPath.turnExternalProfileRightToLeft(zi, rsafe, rc, bci, bcf, path, dsfinishing, Nfinishing, stepR, feedrateR, feedrateZ, feedrateFinishing)
    ...
    def readFile(self, filename):
        ...
        if contour is None:
            ...
            rsafe = extractValue("Xs",formfields)
            rc = extractValue("Rt",formfields)
            listOfErrors = LatheMonotonicPath.isTurnExternalProfileRightToLeftFeasible(zi, rsafe, rc, contour)
            for err in listOfErrors:
                tkMessageBox.showerror(title="Erro", message=err)
        return contour

if __name__ == "__main__":
    app = LatheMonoExternalProfileRightToLeftWidget(None)
    app.grid()
